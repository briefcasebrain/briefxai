...
mod tests {
    ...
    fn test_format_validator() {
        use briefx::preprocessing::validators::{FormatValidator, IssueType, Validator};
        ...
        assert!(result
            .issues
            .iter()
            .any(|i| i.issue_type == IssueType::Empty));
        assert!(result
            .issues
            .iter()
            .any(|i| i.issue_type == IssueType::TooShort));
    }

    #[test]
    fn test_duplicate_detector() {
        use briefx::preprocessing::validators::{DuplicateDetector, IssueType, Validator};
        ...
        assert!(result
            .issues
            .iter()
            .any(|i| i.issue_type == IssueType::Duplicate));
        assert!(result.quality_score < 1.0);
    }

    #[test]
    fn test_pii_detector() {
        use briefx::preprocessing::validators::{IssueType, PIIDetector, Validator};
        ...
        assert!(!result.issues.is_empty());
        assert!(result.issues.iter().any(|i| i.issue_type == IssueType::PII));
    ...
    fn test_language_detector() {
        use briefx::preprocessing::language_detector::LanguageDetector;
        ...
        println!(
            ...
            english.language, english.confidence
        );
    ...
    fn test_unicode_normalizer() {
        use briefx::preprocessing::smart_preprocessor::{Normalizer, UnicodeNormalizer};
        ...
        assert_eq!(normalized.messages[0].content, "café");
    }

    #[test]
    fn test_whitespace_normalizer() {
        use briefx::preprocessing::smart_preprocessor::{Normalizer, WhitespaceNormalizer};
    ...
    async fn test_data_quality_report() {
        use briefx::preprocessing::smart_preprocessor::{PreprocessingOptions, SmartPreprocessor};
    ...
    async fn test_batching_strategy() {
        ...
        assert_eq!(
            strategy.estimated_batches,
            (250 + strategy.batch_size - 1) / strategy.batch_size
        );
    }
}

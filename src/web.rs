...
use askama::Template;
use axum::{
    extract::{ws::{WebSocket, WebSocketUpgrade, Message}, Path as AxumPath, State, Multipart},
...
impl ProgressTracker {
    ...
    pub async fn start_session(&self, session_id: String) -> Result<()> {
        self.sessions.write().await.insert(session_id.clone(), "started".to_string());
        let _ = self.sender.send(ProgressMessage::Started { session_id });
        Ok(())
    }

    pub async fn update_progress(&self, session_id: &str, step: &str, progress: f32, message: &str, details: Option<String>) -> Result<()> {
        ...
        debug!("Progress update for {}: {} - {:.1}% - {}", session_id, step, progress, message);
    ...
    pub async fn complete_session(&self, session_id: String, result: String) -> Result<()> {
        self.sessions.write().await.insert(session_id.clone(), "completed".to_string());
        let _ = self.sender.send(ProgressMessage::Completed { session_id, result });
        Ok(())
    }

    pub async fn error_session(&self, session_id: String, error: String) -> Result<()> {
        self.sessions.write().await.insert(session_id.clone(), "error".to_string());
        let _ = self.sender.send(ProgressMessage::Error { session_id, error });
        Ok(())
    }
...
#[derive(Template)]
#[template(path = "visualization.html", escape = "none")]
...
fn generate_index_html(data_chunks: &[String], password_protected: bool) -> Result<String> {
...
pub async fn serve_website(config: BriefXAIConfig, output_dir: PathBuf) -> Result<()> {
    let app = Router::new()
        ...
        .route("/visualization", get(visualization_handler))
...
pub async fn serve_interactive_ui(config: BriefXAIConfig, ui_dir: PathBuf) -> Result<()> {
    ...
    let app = Router::new()
        .route("/", get_service(ServeDir::new(&ui_dir).append_index_html_on_directories(true)))
        ...
        .route("/api/session/:session_id/status", get(session_status_handler))
...
async fn index_handler() -> impl IntoResponse {
    Html(generate_index_html(&[], false).unwrap_or_default())
}

async fn visualization_handler() -> impl IntoResponse {
    Html(generate_visualization_html().unwrap_or_default())
}
...
async fn analyze_endpoint(
    ...
    config.llm_provider = match request.config.llm_provider.as_str() {
        ...
        "ollama" => crate::config::LlmProvider::Ollama,
        "huggingface" => crate::config::LlmProvider::HuggingFace,
        "vllm" => crate::config::LlmProvider::VLLM,
        _ => crate::config::LlmProvider::OpenAI,
    };
    
    config.llm_model = request.config.llm_model;
    config.llm_api_key = request.config.api_key.clone();
    ...
    config.embedding_provider = match request.config.embedding_provider.as_str() {
        "openai" => crate::config::EmbeddingProvider::OpenAI,
        "sentence-transformers" => crate::config::EmbeddingProvider::SentenceTransformers,
        "huggingface" => crate::config::EmbeddingProvider::HuggingFace,
        "ollama" => crate::config::EmbeddingProvider::OpenAI, // Fallback for now
        _ => crate::config::EmbeddingProvider::OpenAI,
    };
    
    config.embedding_model = request.config.embedding_model;
    config.embedding_api_key = request.config.embedding_api_key.or(request.config.api_key.clone());
    ...
    let result = run_analysis_with_progress(config, request.data, session_id.clone()).await;
    
    match result {
        Ok(results) => {
            let _ = GLOBAL_PROGRESS.complete_session(session_id.clone(), "Analysis completed successfully".to_string()).await;
        ...
        Err(e) => {
            let _ = GLOBAL_PROGRESS.error_session(session_id.clone(), e.to_string()).await;
            AxumJson(ApiResponse {
                success: false,
                data: Some(serde_json::json!({
                    "session_id": session_id
...
async fn start_server_endpoint(
    ...
    let result = match req.server_type.as_str() {
        "ollama" => {
                ...
                .spawn()
            {
                Ok(_) => {
                    ...
                    Ok("Ollama server started successfully. You may need to pull a model.".to_string())
                }
                Err(e) => {
                    if e.kind() == std::io::ErrorKind::NotFound {
                        ...
                        Err(anyhow::anyhow!("Failed to start Ollama: {}. Is another instance already running?", e))
                    }
                }
            }
        }
        "vllm" => {
            ...
            let python_cmd = if Command::new("python3").arg("--version").output().is_ok() {
                ...
                return AxumJson(ApiResponse {
                    success: false,
                    data: None,
                    error: Some("Python not found. Please install Python 3.8 or later.".to_string()),
                });
            };
        ...
        _ => {
            Err(anyhow::anyhow!("Unknown server type: {}", req.server_type))
        }
    };
...
async fn run_analysis_with_progress(
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        &session_id,
        "validation",
        2.0,
        "Starting data validation",
        Some(format!("Checking {} conversations for validity", data.len()))
    ).await;
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        ...
        Some(format!("Messages per conversation: min={}, avg={}, max={}", 
    ...
    let processed_data = if config.dedup_data {
        ...
        let _ = GLOBAL_PROGRESS.update_progress(
            ...
            15.0,
            "Validation complete",
            Some(format!("✓ {} unique conversations ready for analysis", data.len()))
        ).await;
        data
    };
    ...
    for (i, chunk) in processed_data.chunks(batch_size).enumerate() {
        ...
        let _ = GLOBAL_PROGRESS.update_progress(
            ...
            Some(format!("Analyzing batch {}/{} ({} conversations)", 
                i + 1, total_batches, chunk.len()))
        ).await;
        ...
        match crate::facets::extract_facets(&config, chunk).await {
            ...
            Err(e) => {
                ...
                warn!("Error in batch {}: {}. Retrying with smaller batch...", i + 1, e);
                ...
                for (j, single_conv) in chunk.iter().enumerate() {
                    let _ = GLOBAL_PROGRESS.update_progress(
                        ...
                        Some(format!("Processing conversation {} of batch {}", j + 1, i + 1))
                    ).await;
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        ...
        Some(format!("✓ Extracted {} facet categories from conversations", facet_count))
    ).await;
    ...
    for (i, chunk) in processed_data.chunks(embed_batch_size).enumerate() {
        ...
        let _ = GLOBAL_PROGRESS.update_progress(
            ...
            Some(format!("Processing batch {}/{} ({} items)",
                i + 1, total_embed_batches, chunk.len()))
        ).await;
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        ...
        Some(format!("✓ Generated {} embeddings ({} dimensions)", 
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        ...
        Some(format!("Computing {} clusters for {} conversations", 
            n_clusters, processed_data.len()))
    ).await;
    ...
    let clusters = crate::clustering::create_base_clusters(&config, &embeddings, &facet_data).await?;
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        ...
        70.0,
        "Clustering complete",
        Some(format!("✓ Created {} clusters (avg size: {} conversations)", 
            clusters.len(), avg_size))
    ).await;
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        ...
        80.0,
        "Hierarchy complete",
        Some(format!("✓ Built hierarchical structure with {} top-level clusters", 
            hierarchy.len()))
    ).await;
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        ...
        Some(format!("Reducing {}-dimensional embeddings to 2D", embedding_dim))
    ).await;
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        ...
        Some(format!("✓ Projected {} points to 2D visualization space", umap_coords.len()))
    ).await;
    ...
    let _ = GLOBAL_PROGRESS.update_progress(
        ...
        100.0,
        "Analysis complete!",
        Some(format!("✓ Successfully analyzed {} conversations", processed_data.len()))
    ).await;
        ...
        match field.bytes().await {
            Ok(bytes) => {
                ...
                match serde_json::from_slice::<Vec<crate::types::ConversationData>>(&bytes) {
                    Ok(data) => {
                        info!("Successfully parsed {} conversations from {}", data.len(), file_name);
                        ...
                        match serde_json::from_slice::<crate::types::ConversationData>(&bytes) {
                            ...
                            Err(_) => {
                                ...
                                match parse_csv_conversations(&bytes) {
                                    Ok(data) => {
                                        info!("Successfully parsed {} conversations from CSV {}", data.len(), file_name);
                                        conversation_data.extend(data);
                                        uploaded_files.push(FileUploadInfo {
                                            name: file_name.clone(),
                                            size: bytes.len(),
                                    ...
                                    Err(_) => {
                                        ...
                                        match parse_text_conversation(&bytes) {
                                            Ok(data) => {
                                                info!("Successfully parsed text conversation from {}", file_name);
                                                conversation_data.push(data);
                                                uploaded_files.push(FileUploadInfo {
                                                    name: file_name.clone(),
                                                    size: bytes.len(),
                                            ...
                                            Err(_) => {
                                                let error_msg = format!("Could not parse file '{}': {}", file_name, e);
                                                warn!("{}", error_msg);
...
fn parse_csv_conversations(bytes: &[u8]) -> Result<Vec<crate::types::ConversationData>> {
    ...
    for line in content.lines().skip(1) { // Skip header if exists
        ...
        if parts.len() >= 3 {
            ...
            current_id = conv_id;
            current_conversation.push(crate::types::Message {
                role,
                content,
            });
        }
...
fn parse_text_conversation(bytes: &[u8]) -> Result<crate::types::ConversationData> {
    ...
    for line in content.lines() {
        ...
        if line.starts_with("User:") || line.starts_with("user:") || line.starts_with("Human:") {
            ...
            current_content = line.split_once(':').map(|(_, c)| c.trim().to_string()).unwrap_or_default();
        } else if line.starts_with("Assistant:") || line.starts_with("assistant:") || line.starts_with("AI:") {
            if !current_content.is_empty() {
                messages.push(crate::types::Message {
                    role: current_role,
                    content: current_content.trim().to_string(),
...
async fn progress_endpoint(
    ...
    let status = GLOBAL_PROGRESS.sessions.read().await.get(&session_id).cloned();

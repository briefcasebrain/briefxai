...
use crate::visualization::interactive_map::{InteractiveMap, FacetOverlay, ColorScheme, AggregationType};
use crate::privacy::threshold_protection::{PrivacyFilter, PrivacyConfig, PrivacyReport};
use crate::investigation::targeted_search::{
    InvestigationEngine, InvestigationQuery, InvestigationResult, SortCriterion,
    MetricFilter, FacetFilter, ClusterMetric, ComparisonOperator, FilterOperator
};
use crate::discovery::serendipitous::{DiscoveryEngine, DiscoveryRecommendation, UserPreferences, RecommendationType};
...
pub fn create_clio_routes() -> Router {
    Router::new()
        ...
        .route("/api/clio/investigate/suggest", get(get_investigation_suggestions))
        
        // Discovery routes
        .route("/api/clio/discovery/recommendations", post(get_discovery_recommendations))
        .route("/api/clio/discovery/update_preferences", post(update_discovery_preferences))
...
async fn create_interactive_map(
    ...
    info!("Creating interactive map with {} clusters", request.clusters.len());
    ...
    match InteractiveMap::new(request.clusters, request.umap_coords, &request.facet_data) {
        ...
        Err(e) => {
            Ok(AxumJson(ApiResponse::error(format!("Failed to create map: {}", e))))
        }
    }
}
...
async fn apply_facet_overlay(
    ...
    let color_scheme = match request.color_scheme.as_str() {
        ...
        _ => return Ok(AxumJson(ApiResponse::error("Invalid color scheme".to_string()))),
    };
    ...
    let aggregation = match request.aggregation.as_str() {
        ...
        _ => return Ok(AxumJson(ApiResponse::error("Invalid aggregation type".to_string()))),
    };
    ...
    if let Ok(mut instances) = MAP_INSTANCES.lock() {
        if let Some(map) = instances.get_mut(&map_id) {
            ...
            match map.apply_facet_overlay(overlay) {
                Ok(_) => Ok(AxumJson(ApiResponse::success("Overlay applied".to_string()))),
                Err(e) => Ok(AxumJson(ApiResponse::error(format!("Failed to apply overlay: {}", e)))),
            }
        } else {
            Ok(AxumJson(ApiResponse::error("Map not found".to_string())))
        }
    } else {
        Ok(AxumJson(ApiResponse::error("Failed to access map instances".to_string())))
    }
}
    ...
    if let Ok(instances) = MAP_INSTANCES.lock() {
        if let Some(map) = instances.get(&map_id) {
            ...
            match serde_json::to_value(data) {
                ...
                Err(e) => Ok(AxumJson(ApiResponse::error(format!("Serialization error: {}", e)))),
            }
        } else {
            Ok(AxumJson(ApiResponse::error("Map not found".to_string())))
        }
...
async fn apply_privacy_filter(
    ...
    match filter.filter_clusters(request.clusters, &request.conversations).await {
        Ok(filtered_clusters) => {
            let report = filter.generate_report();
            Ok(AxumJson(ApiResponse::success((filtered_clusters, report))))
        }
        Err(e) => {
            Ok(AxumJson(ApiResponse::error(format!("Privacy filtering failed: {}", e))))
        }
    }
}
...
async fn run_investigation(
    ...
    let engine = match InvestigationEngine::new(
        ...
        Ok(engine) => engine,
        Err(e) => return Ok(AxumJson(ApiResponse::error(format!("Failed to create investigation engine: {}", e)))),
    };
    ...
    let facet_filters: Result<Vec<FacetFilter>, String> = request.query.facet_filters
        .into_iter()
        .map(|f| {
    ...
    let metric_filters: Result<Vec<MetricFilter>, String> = request.query.metric_filters
        .into_iter()
        .map(|m| {
    ...
    match engine.investigate(&query) {
        ...
        Err(e) => Ok(AxumJson(ApiResponse::error(format!("Investigation failed: {}", e)))),
    }
}
...
async fn get_discovery_recommendations(
    ...
    let engine = match DiscoveryEngine::new(request.clusters, request.facet_data) {
        Ok(engine) => engine,
        Err(e) => return Ok(AxumJson(ApiResponse::error(format!("Failed to create discovery engine: {}", e)))),
    };
    ...
    match engine.get_recommendations(request.current_cluster, limit) {
        ...
        Err(e) => Ok(AxumJson(ApiResponse::error(format!("Failed to get recommendations: {}", e)))),
    }
}
...
async fn update_discovery_preferences(
    ...
    Json(visited_sequence): Json<Vec<usize>>,
) -> Result<AxumJson<ApiResponse<String>>, StatusCode> {
    if let Ok(mut engines) = DISCOVERY_ENGINES.lock() {
        if let Some(engine) = engines.get_mut(&engine_id) {
            ...
            Ok(AxumJson(ApiResponse::success("Preferences updated".to_string())))
        } else {
            Ok(AxumJson(ApiResponse::error("Discovery engine not found".to_string())))
        }
    } else {
        Ok(AxumJson(ApiResponse::error("Failed to access discovery engines".to_string())))
    }
}

...
pub async fn generate_umap(
    ...
    info!(
        "Generating UMAP-like visualization for {} points",
        embeddings.len()
    );
...
pub async fn generate_pca_projection(
    ...
    let pca_model = pca.fit(&dataset).context("Failed to fit PCA model")?;
...
pub async fn generate_tsne_like_projection(
    ...
    let n_points = embeddings.len();
    let _rng = Xoshiro256Plus::seed_from_u64(seed);
...
pub async fn generate_advanced_umap_projection(
    ...
    info!(
        "Generating advanced UMAP-like projection for {} points with {} neighbors",
...
fn find_k_nearest_neighbors(embeddings: &[Vec<f32>], k: usize) -> Result<Vec<Vec<usize>>> {
    let n_points = embeddings.len();

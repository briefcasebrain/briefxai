...
impl EmbeddingGenerator {
    pub async fn new(config: BriefXAIConfig) -> Result<Self> {
        let provider: Arc<dyn EmbeddingProviderTrait> = match config.embedding_provider {
            EmbeddingProvider::OpenAI => Arc::new(OpenAIEmbeddings::new(
                config.embedding_model.clone(),
                config.embedding_api_key.clone(),
            )?),
            EmbeddingProvider::HuggingFace => Arc::new(
            ...
            EmbeddingProvider::SentenceTransformers => {
                Arc::new(OllamaEmbeddings::new(config.embedding_model.clone()).await?)
            }
            _ => Arc::new(OllamaEmbeddings::new(config.embedding_model.clone()).await?),
        };

        info!(
            "Initialized {} embedding provider with model {}",
            provider.name(),
    ...
    pub async fn generate_batch(
        ...
        for chunk in conversations.chunks(batch_size) {
            let texts: Vec<String> = chunk.iter().map(conversation_to_string).collect();
            ...
            if !texts_to_compute.is_empty() {
                ...
                new_embeddings = self
                    .provider
                    .generate(texts_to_compute.clone())
                    .await
                    .with_context(|| {
                        format!(
                            "Failed to generate embeddings for batch of {}",
                            texts_to_compute.len()
                        )
                    })?;
...
impl EmbeddingProviderTrait for OllamaEmbeddings {
    async fn generate(&self, texts: Vec<String>) -> Result<Vec<Vec<f32>>> {
        ...
        for text in texts {
            let request =
                GenerateEmbeddingsRequest::new(self.model.clone(), EmbeddingsInput::Single(text));
...
mod tests {
    ...
    fn test_normalize_embeddings() {
        let mut embeddings = vec![vec![3.0, 4.0], vec![1.0, 0.0]];
    ...
    fn test_normalize_embeddings_zero_vector() {
        let mut embeddings = vec![vec![0.0, 0.0], vec![5.0, 12.0]];
    ...
    async fn test_openai_embeddings_no_key() {
        ...
        let result = OpenAIEmbeddings::new("text-embedding-3-small".to_string(), None);
    ...
    async fn test_huggingface_embeddings_dimensions() {
        let embeddings =
            HuggingFaceEmbeddings::new("sentence-transformers/all-mpnet-base-v2".to_string(), None)
                .await
                .unwrap();
        ...
        let embeddings_minilm =
            HuggingFaceEmbeddings::new("sentence-transformers/all-MiniLM-L6-v2".to_string(), None)
                .await
                .unwrap();
    ...
    fn test_embedding_provider_names() {
        ...
        assert_eq!(
            EmbeddingProvider::SentenceTransformers.to_string(),

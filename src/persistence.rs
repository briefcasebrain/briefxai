...
impl PersistenceLayer {
    ...
    pub async fn get_cached_llm_response(
        ...
        if let Some(row) = row {
            ...
            // Store in memory cache
            self.memory_cache
                .set(cache_key, response.as_bytes().to_vec(), 3600);
    ...
    pub async fn get_analysis_results(&self, session_id: &str) -> Result<Option<AnalysisResults>> {
        let row = sqlx::query(
            r#"
    ...
    pub async fn import_session(&self, export: SessionExport) -> Result<String> {
        ...
        // Create session
        self.create_session(&new_session_id, &export.session.config)
            .await?;

        // Store conversations
        self.store_conversations(&new_session_id, &export.conversations)
            .await?;

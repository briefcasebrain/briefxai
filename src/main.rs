...
use briefxai::{BriefXAI, BriefXAIConfig, types::{ConversationData, Message}};
use serde_json;
...
async fn start_ui_mode(mut config: BriefXAIConfig, port: u16, open_browser: bool) -> Result<()> {
    ...
    briefxai::web::serve_interactive_ui(config, ui_dir).await?
...
async fn analyze_from_cli(config: BriefXAIConfig, input: PathBuf, output: PathBuf) -> Result<()> {
    ...
    let results = briefxai.run(data, &output).await?;
...
async fn serve_existing(config: BriefXAIConfig, directory: PathBuf, port: u16) -> Result<()> {
    ...
    briefxai::web::serve_website(config, directory).await?
    Ok(())
}
...
async fn example_command(output: PathBuf, count: usize) -> Result<()> {
    ...
    let conversations = briefxai::examples::generate_realistic_conversations(count, Some(42));

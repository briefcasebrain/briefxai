...
use crate::prompts::{
    get_facet_cluster_name_prompt,
    get_neighborhood_cluster_names_prompt,
    get_assign_to_high_level_cluster_prompt,
    get_renaming_higher_level_cluster_prompt,
};
use crate::types::{ConversationCluster, FacetValue, Facet};
use crate::utils::{sample_items, most_common};
...
pub async fn create_base_clusters(
    ...
    info!("Creating base clusters for {} data points", embeddings.len());
...
async fn generate_cluster_name_description(
    ...
    let final_desc = most_common(descriptions).unwrap_or_else(|| "No description available".to_string());
...
pub async fn build_hierarchy(
    ...
    info!("Building cluster hierarchy from {} base clusters", base_clusters.len());
    ...
    while current_level.len() > config.min_top_level_size {
        info!("Building next hierarchy level from {} clusters", current_level.len());
        
        let next_level = build_next_level(
            config,
            &llm_client,
            current_level.clone(),
        ).await?;
...
async fn build_next_level(
    ...
    for (facet, facet_clusters) in clusters_by_facet {
        let higher_clusters = create_higher_level_clusters(
            config,
            llm_client,
            &facet,
            facet_clusters,
...
async fn create_higher_level_clusters(
    ...
    for neighborhood in neighborhoods {
        ...
        let neighborhood_clusters = create_neighborhood_clusters(
            config,
            llm_client,
            facet,
            neighborhood,
...
async fn create_neighborhood_clusters(
    ...
    let assignments = assign_to_categories(
        llm_client,
        &neighborhood,
        &higher_level_names,
    ).await?;
    ...
    for (i, (name, desc)) in higher_level_names.iter().enumerate() {
        ...
        let refined = refine_cluster_name(
            llm_client,
            name,
            desc,
            &children,
            config.n_rename_samples,
        ).await?;
        ...
        if lines[i].contains("Name:") {
            if let Some(name) = lines[i].split("Name:").nth(1) {
                ...
                let desc = if i + 1 < lines.len() && lines[i + 1].contains("Description:") {
                    i += 1;
                    lines[i].split("Description:").nth(1).map(|s| s.trim().to_string())
...
fn parse_assignments(response: &str, n_clusters: usize) -> Result<Vec<usize>> {
    ...
    for line in response.lines() {
        if let Some((cluster_str, category_str)) = line.split_once("->") {
            let cluster_idx = cluster_str
                .trim()
                .parse::<usize>()
                .ok()
                .and_then(|i| if i > 0 { Some(i - 1) } else { None });
            
            let category_idx = category_str
                .trim()
                .chars()
                .next()
                .and_then(|c| {
...
mod tests {
    ...
    fn create_test_embeddings(n: usize) -> Vec<Vec<f32>> {
        (0..n).map(|i| {
            vec![i as f32 / n as f32, (n - i) as f32 / n as f32]
        }).collect()
    }
    ...
    fn test_sample_cluster_points() {
        ...
        let facet_data = vec![
            vec![FacetValue { facet: facet.clone(), value: "positive".to_string() }],
            vec![FacetValue { facet: facet.clone(), value: "negative".to_string() }],
            vec![FacetValue { facet: facet.clone(), value: "neutral".to_string() }],
        ];
        ...
        assert!(samples.contains(&"positive".to_string()) || 
...
pub fn perform_kmeans(embeddings: &[Vec<f32>], n_clusters: usize) -> Result<Vec<usize>> {
    ...
    info!("Running K-means clustering with {} clusters for {} points", actual_clusters, n_points);
    
    // Convert embeddings to ndarray format
    let mut data = Array2::zeros((n_points, dim));
    for (i, embedding) in embeddings.iter().enumerate() {
        if embedding.len() != dim {
            warn!("Embedding {} has dimension {} but expected {}, skipping", i, embedding.len(), dim);
            continue;
        }
    ...
    match kmeans.fit(&dataset) {
        Ok(model) => {
            ...
            let assignments: Vec<usize> = predictions.as_targets()
                .iter()
                .map(|&x| x as usize)
                .collect();
            ...
            if max_cluster >= actual_clusters {
                warn!("Invalid cluster assignment detected: max cluster {} >= {}", max_cluster, actual_clusters);
            }
            
            Ok(assignments)
        }
        Err(e) => {
            warn!("K-means clustering failed: {}, falling back to simple assignment", e);
            
            // Fallback: simple round-robin assignment
            let assignments: Vec<usize> = (0..n_points)
                .map(|i| i % actual_clusters)
                .collect();

...
use crate::types::{ConversationData, FacetValue, ConversationCluster};
use crate::types_extended::AnalysisCluster;
use crate::privacy::threshold_protection::{PrivacyFilter, PrivacyConfig, PrivacyReport};
...
impl EnhancedAnalysisEngine {
    ...
    pub async fn analyze_with_clio_features(
        ...
        info!("Starting enhanced analysis with Clio features for {} conversations", 
              conversations.len());
        ...
        if conversations.len() < self.clio_config.min_dataset_size {
            warn!("Dataset size {} below minimum {} for Clio features", 
                  conversations.len(), self.clio_config.min_dataset_size);
        ...
        let (privacy_filtered_clusters, privacy_report) = if self.clio_config.enable_privacy_filtering {
            self.apply_privacy_filtering(&clusters, &conversations).await?
        } else {
            (clusters.clone(), PrivacyReport {
                total_clusters: clusters.len(),
                filtered_clusters: 0,
                small_clusters_merged: 0,
                sensitive_facets_filtered: vec![],
        ...
        info!("Privacy filtering: {} clusters -> {} clusters", 
              clusters.len(), privacy_filtered_clusters.len());
        
        // Create interactive map
        let interactive_map = if self.clio_config.enable_interactive_map {
            self.create_interactive_map(&privacy_filtered_clusters, &umap_coords, &facet_data).await?
        } else {
            None
        };
        ...
        let investigation_engine = if self.clio_config.enable_investigation {
            self.create_investigation_engine(&privacy_filtered_clusters, &conversations, &facet_data, &embeddings).await?
        } else {
            None
        };
        ...
        let discovery_engine = if self.clio_config.enable_discovery {
            self.create_discovery_engine(&privacy_filtered_clusters, &facet_data).await?
        } else {
            None
        };
        
        let visible_conversations = privacy_filtered_clusters.iter()
            .map(|c| c.conversation_ids.len())
            .sum();
        
        let privacy_level = self.assess_privacy_level(&privacy_report);
        
        info!("Enhanced analysis complete: {} visible conversations, privacy level: {}", 
    ...
    async fn apply_privacy_filtering(
        ...
        let filtered_clusters = privacy_filter.filter_clusters(
            clusters.to_vec(),
            conversations,
        ).await?;
    ...
    async fn create_investigation_engine(
        ...
        info!("Creating investigation engine for {} clusters", clusters.len());
    ...
    fn assess_privacy_level(&self, report: &PrivacyReport) -> String {
        ...
        if report.small_clusters_merged > 0 || report.filtered_clusters > 0 {
            format!("{} (filtered {} clusters)", level, report.small_clusters_merged + report.filtered_clusters)
        } else {
            level.to_string()
        }
    }
...
pub mod integration_helpers {
    ...
    pub fn extract_clio_config(config: &BriefXAIConfig) -> ClioConfig {
        ClioConfig {
            enable_privacy_filtering: true,
    ...
    pub fn generate_analysis_summary(results: &EnhancedAnalysisResults) -> HashMap<String, serde_json::Value> {
        let mut summary = HashMap::new();
        
        summary.insert("total_conversations".to_string(), 
                      serde_json::Value::Number(results.total_conversations.into()));
        summary.insert("visible_conversations".to_string(), 
                      serde_json::Value::Number(results.visible_conversations.into()));
        summary.insert("total_clusters".to_string(), 
                      serde_json::Value::Number(results.clusters.len().into()));
        summary.insert("filtered_clusters".to_string(), 
                      serde_json::Value::Number(results.privacy_filtered_clusters.len().into()));
        summary.insert("privacy_level".to_string(), 
                      serde_json::Value::String(results.privacy_level.clone()));
        summary.insert("has_interactive_map".to_string(), 
                      serde_json::Value::Bool(results.interactive_map.is_some()));
        summary.insert("has_investigation".to_string(), 
                      serde_json::Value::Bool(results.investigation_engine.is_some()));
        summary.insert("has_discovery".to_string(), 
                      serde_json::Value::Bool(results.discovery_engine.is_some()));
        
        // Privacy metrics
        summary.insert("privacy_merged_clusters".to_string(), 
                      serde_json::Value::Number(results.privacy_report.small_clusters_merged.into()));
        summary.insert("privacy_redacted_clusters".to_string(), 
...
mod tests {
    ...
    fn create_test_data() -> (Vec<ConversationData>, Vec<Vec<FacetValue>>, Vec<ConversationCluster>) {
        let conversations = vec![
            ConversationData {
                messages: vec![
                    Message { role: "user".to_string(), content: "Help with billing".to_string() },
                    Message { role: "assistant".to_string(), content: "I can help with billing".to_string() },
                ],
                metadata: Default::default(),
            },
            ConversationData {
                messages: vec![
                    Message { role: "user".to_string(), content: "Technical issue".to_string() },
                    Message { role: "assistant".to_string(), content: "Let me assist".to_string() },
                ],
                metadata: Default::default(),
            },
        ];

...
impl LoadBalancer {
    ...
    pub async fn select_provider(&self, providers: &[ManagedProvider]) -> Option<usize> {
...
impl ManagedProvider {
    pub fn new(config: ProviderConfig, provider: Box<dyn LlmProviderTrait>) -> Self {
        ...
        if health.consecutive_failures >= self.circuit_breaker.failure_threshold {
            ...
            if *state != CircuitState::Open {
                ...
                *self.last_state_change.write().await = Instant::now();
                ...
                error!(
                    "Provider {} circuit breaker opened after {} failures",
    ...
    pub async fn health_check(&self) -> Result<HealthStatus> {
        ...
        match self
            .provider
            .complete(test_prompt, &Default::default())
            .await
        {
            Ok(_) => {
                self.record_success(start.elapsed()).await;
...
impl ProviderManager {
    ...
    async fn create_provider(config: &ProviderConfig) -> Result<Box<dyn LlmProviderTrait>> {
        match config.provider_type {
            ProviderType::OpenAI => {
                ...
                let provider =
                    crate::llm::OpenAIProvider::new(model.to_string(), Some(api_key.to_string()))?;
                Ok(Box::new(provider))
            }
            ProviderType::Ollama => {
                let model = config
                ...
                let provider = crate::llm::OllamaProvider::new(model.to_string()).await?;
                Ok(Box::new(provider))
            }
            _ => bail!(
    ...
    pub async fn execute_with_fallback(&self, prompt: &str) -> Result<String> {
        ...
        if let Some(index) = self.load_balancer.select_provider(&self.providers).await {
            ...
            match provider
                .execute(|p| async move { p.complete(prompt, &Default::default()).await })
                .await
            {
                Ok(result) => {
                ...
                Err(e) => {
                    ...
                    self.record_usage(&provider.config.id, false, Some(e.to_string()))
                        .await;
                }
            }
        }

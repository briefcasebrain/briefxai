...
impl LlmClient {
    pub async fn new(config: BriefXAIConfig) -> Result<Self> {
        let provider: Arc<dyn LlmProviderTrait> = match config.llm_provider {
            ...
            LlmProvider::Ollama => Arc::new(OllamaProvider::new(config.llm_model.clone()).await?),
            LlmProvider::VLLM => Arc::new(VLLMProvider::new(
                config.llm_model.clone(),
                config
                    .llm_base_url
                    .clone()
            ...
            LlmProvider::Gemini => Arc::new(GeminiProvider::new(
                config.llm_model.clone(),
                config.llm_api_key.clone(),
            )?),
            _ => {
                // Default to Ollama
                Arc::new(OllamaProvider::new(config.llm_model.clone()).await?)
            }
        };

        info!(
            "Initialized {} LLM provider with model {}",
            provider.name(),
            config.llm_model
        );
    ...
    pub async fn complete_batch(&self, prompts: Vec<String>) -> Result<Vec<String>> {
        ...
        for chunk in prompts.chunks(batch_size) {
            ...
            let futures: Vec<_> = chunk.iter().map(|prompt| self.complete(prompt)).collect();
...
impl OllamaProvider {
    async fn new(model: String) -> Result<Self> {
        ...
        if !model_exists {
            warn!(
                "Model {} not found locally. You may need to pull it first",
                model
            );
        }
...
impl GeminiProvider {
    fn new(model: String, api_key: Option<String>) -> Result<Self> {
        let api_key = api_key
            ...
            .context("Gemini API key not provided (set GEMINI_API_KEY or GOOGLE_API_KEY)")?;
...
impl LlmProviderTrait for GeminiProvider {
    async fn complete(&self, prompt: &str, params: &LlmParams) -> Result<String> {
        ...
        let request = GeminiRequest {
            ...
            generation_config: GeminiGenerationConfig {
                temperature: params.temperature.unwrap_or(0.7),
                max_output_tokens: params.max_tokens.map(|t| t as u32).or(Some(1000)),
            },
        };
    ...
    async fn complete_with_system(
        ...
        // Gemini uses a different approach for system messages
        // We'll combine them in the user message
